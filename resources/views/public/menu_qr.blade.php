        ...
        <div class="hero">
            <div class="surface">
                <div class="surface-body">
                    <div class="brand-box">
                        <img class="logo" src="{{ !empty($corporateData['logo_enterprise']) ? asset($corporateData['logo_enterprise']) : asset('img/sin_imagen.jpg') }}" alt="Logo empresa">
                        <div>
                            <h1 class="title">{{ $corporateData['fantasy_name_enterprise'] ?? ($corporateData['name_enterprise'] ?? 'Menú QR') }}</h1>
                            <p class="subtitle">{{ $corporateData['name_enterprise'] ?? '' }}</p>
                            <div class="meta">
        ...
        @if(count($categorias) > 0)
            <nav class="category-nav">
                @foreach($categorias as $categoria)
                    <a href="#cat-{{ $categoria['id'] }}">{{ $categoria['nombre'] }} <span>({{ count($categoria['items']) }})</span></a>
                @endforeach
            </nav>
        @endif
        ...
        @forelse($categorias as $categoria)
            <section id="cat-{{ $categoria['id'] }}" class="menu-section">
                <div class="menu-section-title">
                    ...
                    <span>{{ count($categoria['items']) }} opciones</span>
                </div>

                <div class="items-grid">
                    @foreach($categoria['items'] as $item)
                        <article class="item-card {{ $item['disponible'] ? '' : 'unavailable' }}">
                            <img class="item-image" src="{{ $item['imagen'] }}" alt="{{ $item['nombre'] }}">
                            <div class="item-content">
                                ...
                                <div class="item-footer">
                                    ...
                                    <span class="badge-state {{ $item['disponible'] ? 'ok' : 'off' }}">{{ $item['disponible'] ? 'Disponible' : 'No disponible' }}</span>
                                </div>
                            </div>
                        </article>
                    @endforeach
