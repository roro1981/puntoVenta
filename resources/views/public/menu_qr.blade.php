...
<div class="qr-menu" data-animation-style="{{ $menuAnimStyle }}" style="{{ $menuThemeStyle }}; {{ $menuVisualStyle }}">
    <div class="qr-shell">
        <header class="qr-hero">
            <div class="qr-brand">
                <img class="qr-logo" src="{{ !empty($corporateData['logo_enterprise']) ? asset($corporateData['logo_enterprise']) : asset('img/sin_imagen.jpg') }}" alt="Logo empresa">
                <div>
                    <h1 class="qr-title">{{ $corporateData['fantasy_name_enterprise'] ?? ($corporateData['name_enterprise'] ?? 'Menú QR') }}</h1>
                    <p class="qr-subtitle">{{ $corporateData['name_enterprise'] ?? '' }}</p>
                    <div class="qr-meta">
                        <div>{{ $corporateData['address_enterprise'] ?? '' }}</div>
                        <div>{{ $corporateData['comuna_enterprise'] ?? '' }}</div>
                        <div>{{ $corporateData['phone_enterprise'] ?? '' }}</div>
                    </div>
                </div>
            </div>
        </header>
                    ...
                    <div class="category-grid" id="categoryGrid">
                        @foreach($categorias as $categoria)
                            ...
                            >
                                ...
                                <span class="category-count">{{ count($categoria['items']) }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
            ...
            <section id="productsView" class="qr-panel products-view">
                <div class="qr-panel-body">
                    ...
                    <div class="category-strip" id="categoryStrip">
                        @foreach($categorias as $categoria)
                            <button type="button" class="category-pill" data-category-id="{{ $categoria['id'] }}">{{ $categoria['nombre'] }}</button>
                        @endforeach
                    </div>

                    @foreach($categorias as $categoria)
                        <section class="category-products" data-category-id="{{ $categoria['id'] }}" data-category-name="{{ $categoria['nombre'] }}">
                            <div class="products-list">
                                @foreach($categoria['items'] as $item)
                                    <article class="product-row {{ $item['disponible'] ? '' : 'unavailable' }}">
                                        <div class="product-info">
                                            ...
                                            <div class="product-meta">
                                                <span class="product-price">${{ number_format($item['precio'], 0, ',', '.') }}</span>
                                                <span class="product-status {{ $item['disponible'] ? 'ok' : 'off' }}">{{ $item['disponible'] ? 'Disponible' : 'No disponible' }}</span>
                                            </div>
                                        </div>

                                        <div class="product-image-wrap">
                                            @if(($item['tipo'] ?? null) === 'product' && !empty($item['popular']))
