 ...
 *
 * 1. Do NOT use the '%' character in any of the fields, it will not work
 * 2. Modified to work with Google Chrome
 */
...
{
	var browser=navigator.appName;					// Added by 3AS : detect browser (Chrome/IE)
	if(sfw[instance].name && sfw[instance].action)
	{
		...
		// Added by 3AS : correct margins depending on browser
		if (browser=='Netscape') 
		{
			sfw[instance].outputContainer.style.marginTop= "24";
		}
...
function sfwQuery(instance)
{
	...
	// Added : Ability to load products based on Vendor for Adquisiciones
	var vid = 0;
	if (document.getElementById("avid2") != null)
	{
		vid = document.po.avid2.value;
	}
	var url = sfw[instance].action + "?type=" + sfw[instance].name + "&q=" + phrase.trim().encode() + "&vid=" + vid;
	// end modif
...
{
	if (sfw[instance].suggestions != null) 
		if(typeof sfw[instance].suggestions != "undefined" && sfw[instance].suggestions.length != null && sfw[instance].suggestions.length)
...
function sfwSuggest(instance, list)
{
	...
	var table = '<table style="width: 100%; margin: 0; padding: 0;font-size: 16px;" cellspacing="0" cellpadding="0">';
	if (sfw[instance].suggestions != null) 
		if(sfw[instance].heading && sfw[instance].suggestions.length != null && sfw[instance].suggestions.length)
		{
			...
			for(var i = 0; i < sfw[instance].columns; i++)
			{
				...
				if(sfw[instance].columns > 1 && i == sfw[instance].columns - 1)
					column += ' style="text-align: right;font-size: 16px;"';
